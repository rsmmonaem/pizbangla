...
class Add_institute_model  extends CI_Model {
    ...
    function get_institute_home() {
        $this->db->order_by("inst_id", "DESC");
        $query = $this->db->get("institute", 4);
        return $query->result();
    }


      function get_institute_details_id($table, $id)
    {
	...
	function get_depertment_frn_key($table, $frn_key)
    {
		$result = $this->db->get_where($table, ['frn_key' => $frn_key])->row();
        return $result;
    }
